...
export const POLICY_URL = "https://www.primeswimacademy.com/school-policy";

export type TuitionEmailVariant = "invoice" | "reminder" | "past_due";

export type TuitionEmailPayload = {
  parentName: string;
  parentEmail: string;
  swimmerName: string;
  months: string[];
...
export function monthsLabel(ms: string[]) {
  if (!ms?.length) return "";
  if (ms.length === 1) return ms[0];
  return `${ms.slice(0, -1).join(", ")} and ${ms.slice(-1)[0]}`;
}

export function monthsShort(ms: string[]) {
...
function mainParagraph(data: TuitionEmailPayload, safe: Record<string, string>): string {
  const variant = data.variant ?? "invoice";
  const monthsFragment = safe.months
    ? ` in <strong>${safe.months}</strong>`
    : "";

  if (variant === "past_due") {
    return `<p style="font-size:16px;margin-bottom:24px;color:#475569;">
        Tuition for <strong>${safe.swimmerName}</strong>'s training${monthsFragment} was due on <strong>${safe.dueDate}</strong>
        and is now <strong style="color:#b91c1c;">past due</strong>. Please send payment as soon as possible so we can keep
        ${safe.swimmerName}'s group spot reserved.
        <span style="color:#b91c1c;">
          ⚠️ A $35 late fee applies if tuition is not paid by the original due date via Zelle, or at the first practice for cash payments.
        </span>
      </p>`;
  }
  ...
  if (variant === "reminder") {
    const daysSnippet =
      typeof data.daysUntilDue === "number" && data.daysUntilDue >= 0
        ? data.daysUntilDue === 0
          ? " (<strong>due today</strong>)"
          : ` (in <strong>${data.daysUntilDue}</strong> day${data.daysUntilDue === 1 ? "" : "s"})`
        : "";
    return `<p style="font-size:16px;margin-bottom:24px;color:#475569;">
        This is a quick reminder: tuition for <strong>${safe.swimmerName}</strong>'s training${monthsFragment} is due by <strong>${safe.dueDate}</strong>${daysSnippet}. Please submit payment on time to keep ${safe.swimmerName}'s spot and avoid any interruption.
        <span style="color:#b91c1c;">
  ...
  return `<p style="font-size:16px;margin-bottom:24px;color:#475569;">
        This is a friendly reminder that tuition for <strong>${safe.swimmerName}</strong>'s training${monthsFragment} is now due. Please submit payment by <strong>${safe.dueDate}</strong>
...
function greetingTitle(variant: TuitionEmailVariant): { h2: string; sub: string } {
  if (variant === "past_due") {
    return { h2: "⚠️ Past Due — Tuition", sub: "Action needed" };
  }
  if (variant === "reminder") {
    return { h2: "💳 Tuition Reminder", sub: "Due soon" };
  }
  return { h2: "💳 Tuition Payment Due", sub: "Tuition Reminder" };
}

function closingParagraph(data: TuitionEmailPayload, safe: Record<string, string>): string {
  const variant = data.variant ?? "invoice";
  if (variant === "past_due") {
    return `<p style="font-size:16px;margin:25px 0;color:#475569;">
        Please pay as soon as you can to keep your swimmer's spot. Once paid, a quick reply with confirmation (screenshot is great) helps us update our records.
      </p>`;
  }
  return `<p style="font-size:16px;margin:25px 0;color:#475569;">
...
export function buildTuitionEmailHtml(data: TuitionEmailPayload): string {
  const variant = data.variant ?? "invoice";
...
<title>Tuition - Prime Swim Academy</title>
<style>
...
export function buildTuitionEmailSubject(data: TuitionEmailPayload): string {
  const variant = data.variant ?? "invoice";
  ...
  const dueFmt = fmtDate(data.dueDate);

  if (variant === "past_due") {
    return `Prime Swim Academy — Past due tuition (${monthLabel || "Training"})`;
  }
  if (variant === "reminder" && typeof data.daysUntilDue === "number") {
    if (data.daysUntilDue === 0) {
      return `Prime Swim Academy — Tuition reminder: due today (${dueFmt})`;
    }
    return `Prime Swim Academy — Tuition reminder: due in ${data.daysUntilDue} day${data.daysUntilDue === 1 ? "" : "s"} (${dueFmt})`;
  }
  if (variant === "reminder") {
    return `Prime Swim Academy — Tuition reminder for ${monthLabel || "training"} (${dueFmt})`;
  }
  return `Prime Swim Academy - Tuition for ${monthLabel || "Training"} (Due ${dueFmt})`;
}
