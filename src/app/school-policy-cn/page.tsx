...
const policies = [
  {
    ...
    content: [
      ...
      "持续在 Prime Swim Academy 训练的家庭（即没有去其他队伍、只是跨学期/跨赛季继续报名的家庭）须保持所有信息为最新（医疗/紧急联系人、免责声明与付款信息等），并按俱乐部/LSC 要求完成续签。",
      "曾在本队训练、之后加入其他队伍、再申请回来的泳员，不属于上述续训家庭，须遵循下方《返队泳员重新报名》政策。",
  ...
  },
  {
    ...
    icon: RefreshCw,
    content: [
      "本政策适用于返队泳员：曾在 Prime Swim Academy 训练、之后加入其他队伍、现申请回到本队的学员。",
      "返队泳员重新加入前必须完成测评（evaluation）或试训（tryout），费用为 $100。",
      "泳员恢复在 Prime Swim Academy 的训练时，需支付 $150 重新报名（re-enrollment）费用。该费用用于重新报名流程，包括评估泳员当前发展水平、确定合适的训练组别、保留名额，以及更新其在项目中的分班。",
      "$100 测评/试训费与 $150 重新报名费一经收取，均不予退款。",
      "常规学费按月支付。返队泳员第一次学费须预缴 3 个月。完成该次预缴后，之后的学费恢复为按月支付。这 3 个月的承诺是为了给泳员、教练团队和训练组提供稳定与连贯性，也让教练有足够时间持续跟进泳员进度、制定训练目标，并在 Prime 项目中支持其切实成长。",
    ],
  },
  // 出勤与名额保留（新增）
  {
  ...
  },
  {
    ...
    content: [
      ...
      "Prime Swim Academy 为错过常规小组课的泳员，每月统一提供一次补课机会。补课时间将每月公布于官网/日历，并通过电子邮件/微信通知，家长登录网站后可为学员选择是否参加。",
      "资格：每名泳员每月最多一次补课。",
      "请注意：补课每月统一安排一次，无法为个别学员单独安排额外补课或调整时间，以维护课程公平性和整体安排的稳定性。",
  ...
  },
  {
    ...
    icon: DollarSign,
    content: [
      "若在课程开课前至少两周退出课程，可全额退款。",
      "请注意：退款政策仅适用于课程开始前退出整个课程或团体训练。课程开始后如孩子缺课，无法对单节课退款；如遇医疗紧急情况且提供有效医疗证明，经审核符合条件者可享受按比例退款或获得未来课程学分，但仍需支付 Prime Swim Academy 已发生的泳道费用。",
  ...
  },
  {
    ...
    icon: Shield,
    content: [
      "学员在课程期间不得与其他学员进行任何身体接触，包括但不限于推搡、抓取、击打或任何其他形式的身体互动。",
      "此政策旨在确保所有参与者的安全与舒适，并维护尊重的学习环境。",
      "任何违反此政策的行为可能导致纪律处分，包括停训或开除且不退款。",
      "如学员需要帮助，应请求教练或工作人员协助，而非寻求其他学员的帮助。",
    ],
  },
  {
    id: "safety-health",
