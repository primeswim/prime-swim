// src/app/api/makeup/send-notification/route.ts
...
function buildMakeupNotificationEmailHtml(params: {
  parentName?: string;
  parentEmail: string;
  swimmerName: string;
  eventText: string;
...
<title>Make-up Class Notification</title>
<style>
  *{margin:0;padding:0;box-sizing:border-box;}
  ...
  .welcome-message{background:linear-gradient(135deg,#dbeafe 0%,#bfdbfe 100%);border-left:4px solid #3b82f6;padding:24px;margin-bottom:30px;border-radius:0 12px 12px 0;box-shadow:0 4px 6px -1px rgba(0,0,0,.1);}
  .welcome-message h2{color:#1e40af;font-size:24px;margin-bottom:12px;font-weight:700;}
  ...
  .event-detail{margin:12px 0;font-size:16px;color:#475569;line-height:1.7;}
  .event-detail strong{color:#1e293b;font-weight:600;}
...
<body>
  <div class="container">
    <div class="header">
      ...
      <p>Make-up Class Notification</p>
    </div>
    <div class="content">
      <div class="welcome-message">
        <h2>🏊 Make-up Class Available</h2>
        <p>Dear ${parentName || "Parent/Guardian"},</p>
      </div>
      <p style="font-size:16px;margin-bottom:24px;color:#475569;line-height:1.7;">
        A make-up class has been scheduled for <strong>${swimmerName}</strong>. Please review the details below and let us know if you'll be attending.
      </p>
      <div class="event-info">
        <h3>📅 Make-up Class Details</h3>
        ${dateStr ? `<div class="event-detail"><strong>Date:</strong> ${dateStr}</div>` : ""}
        ${timeStr ? `<div class="event-detail"><strong>Time:</strong> ${timeStr}</div>` : ""}
        ${location ? `<div class="event-detail"><strong>Location:</strong> ${location}</div>` : ""}
        ${eventText ? `<div class="event-detail" style="margin-top:16px;padding-top:16px;border-top:1px solid #e2e8f0;"><strong>Additional Information:</strong><br/>${eventText}</div>` : ""}
      </div>
      <p style="font-size:16px;margin:25px 0;color:#475569;line-height:1.7;">
        Please log in to your Parent Portal to RSVP for this make-up class.
      </p>
...
export async function POST(req: Request) {
  try {
    ...
    const time = eventData.time || null;
    const endTime = eventData.endTime || null;
    ...
    const results = await Promise.allSettled(
      swimmerIds.map(async (swimmerId: string) => {
        ...
        if (!swimmerDoc.exists) {
          return { swimmerId, success: false, error: "Swimmer not found" };
        }
        ...
        if (!parentEmail) {
          return { swimmerId, success: false, error: "No parent email" };
        }
        ...
        await resend.emails.send({
          ...
          html: buildMakeupNotificationEmailHtml({
            parentName,
            parentEmail,
            swimmerName,
            eventText,
