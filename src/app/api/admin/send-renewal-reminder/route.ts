// src/app/api/admin/send-renewal-reminder/route.ts
...
async function requireAdmin(req: NextRequest) {
  ...
  if (!token) return { ok: false as const, status: 401, msg: "Missing Authorization Bearer token" };

  const decoded = await adminAuth.verifyIdToken(token).catch(() => null);
  if (!decoded?.email) return { ok: false as const, status: 401, msg: "Invalid token" };

  const adminDoc = await adminDb.doc(`admin/${decoded.email}`).get();
  if (!adminDoc.exists) return { ok: false as const, status: 403, msg: "Not an admin" };
...
// ===== 样式：复用你之前的风格（略微改标题与正文）
function buildEmailHtml(params: {
  parentName?: string;
  parentEmail: string;
  swimmerName: string;
  nextDueDate?: string | Date | null;
  kind: TemplateKind; // due_soon | grace | inactive
}) {
    ...
    <div class="content">
      ...
      <div class="signature">
        <p style="font-size:16px;color:#64748b;margin-bottom:20px;">
          Thank you for supporting our program. We look forward to seeing ${swimmerName} in the pool! 🌊
        </p>
...
export async function POST(req: NextRequest) {
  try {
    ...
    // 2) 请求体
    const { parentEmail, parentName, swimmerName, status, nextDueDate, testMode } = await req.json();

    // Test mode: send test email to admin email
    if (testMode === true) {
      const testEmail = auth.email; // Send to admin's email
      const testKind: TemplateKind = status === "grace" ? "grace" : status === "inactive" ? "inactive" : "due_soon";
      const testSubject = `[TEST] ${subjectFor(testKind, swimmerName || "Test Swimmer", nextDueDate || new Date())}`;
      const testHtml = buildEmailHtml({
        parentName: parentName || "Test Parent",
        parentEmail: testEmail,
        swimmerName: swimmerName || "Test Swimmer",
      ...
      return NextResponse.json({ ok: true, data: response, message: `Test email sent to ${testEmail}` });
    }

    if (!parentEmail || !swimmerName) {
      return NextResponse.json({ ok: false, error: "Missing parentEmail or swimmerName" }, { status: 400 });
    }
